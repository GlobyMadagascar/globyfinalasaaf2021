...
public class TownActivity extends  AppCompatActivity  { 
	...
	private DatabaseReference lieudb_verification = _firebase.getReference("lieudb_verification");
	private ChildEventListener _lieudb_verification_child_listener;
	private Intent m = new Intent();
	private AlertDialog.Builder di;
	private Intent rdv = new Intent();
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		listview1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				p.putExtra("description", lm.get((int)_position).get("details").toString());
				startActivity(p);
			}
		});
		...
		listview1.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				final int _position = _param3;
				k.setTitle("Chargement");
				k.setMessage("Voulez-vous vraiment charger les données à la base de données ?");
				k.setPositiveButton("Envoyer", new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface _dialog, int _which) {
						k.create().show();
						map = new HashMap<>();
						map.put("nom", lm.get((int)_position).get("nom").toString());
						map.put("phone", lm.get((int)_position).get("phone").toString());
						map.put("description", lm.get((int)_position).get("description").toString());
						map.put("site", lm.get((int)_position).get("site").toString());
						map.put("reg", lm.get((int)_position).get("reg").toString());
						map.put("type", lm.get((int)_position).get("type").toString());
						map.put("lat", lm.get((int)_position).get("lat").toString());
						map.put("lng", lm.get((int)_position).get("lng").toString());
						map.put("username", lm.get((int)_position).get("username").toString());
						map.put("pic", lm.get((int)_position).get("pic").toString());
						map.put("promo", lm.get((int)_position).get("promo").toString());
						map.put("id", FirebaseAuth.getInstance().getCurrentUser().getUid().concat(new SimpleDateFormat("ddMMyyHHmmss").format(c.getTime())));
						map.put("plus1", "");
						map.put("plus2", "");
						map.put("plus3", "");
						map.put("plus4", "");
						map.put("plus5", "");
						map.put("ville", lm.get((int)_position).get("ville").toString());
						lieudb_verification.child(FirebaseAuth.getInstance().getCurrentUser().getUid().concat(new SimpleDateFormat("ddMMyyHHmmss").format(c.getTime()))).updateChildren(map);
				...
				k.setNegativeButton("Retour", new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface _dialog, int _which) {
		...
		_fab.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				m.setAction(Intent.ACTION_CALL);
				m.setData(Uri.parse("tel:".concat("0329402793")));
				startActivity(m);
			}
		});
		
		_lieudb_verification_child_listener = new ChildEventListener() {
			@Override
		...
		lieudb_verification.addChildEventListener(_lieudb_verification_child_listener);
	...
	private void initializeLogic() {
		...
		_ARGHOZALITapTarget(_fab, "CONTACTER UN SPÉCIALISTE", "Vous pouvez appeler un vulgarisateur pour avoir quel astuce vous va au mieux.", "#03a9f4");
	}
	...
	public void _ARGHOZALITapTarget (final View _view, final String _title, final String _msg, final String _bgcolor) {
		TapTargetView.showFor(TownActivity.this,
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			View _view = _v;
			if (_view == null) {
				_view = _inflater.inflate(R.layout.mag, null);
			}
			
			final LinearLayout linear1 = (LinearLayout) _view.findViewById(R.id.linear1);
			final ImageView imageview1 = (ImageView) _view.findViewById(R.id.imageview1);
			final LinearLayout linear2 = (LinearLayout) _view.findViewById(R.id.linear2);
			final TextView nom = (TextView) _view.findViewById(R.id.nom);
			final TextView ville = (TextView) _view.findViewById(R.id.ville);
			
			_Bory(linear1, "#ffffff", "#e0e0e0", 15);
			nom.setText(lm.get((int)_position).get("nom").toString());
			ville.setText(lm.get((int)_position).get("type").toString());
